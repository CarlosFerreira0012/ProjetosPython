...
def borda_caracteres(texto, textoBife, textoFile, textoP, textoM, textoG, ValorOne, ValorTwo, ValorTree, ValorOneF, ValorTwoF, ValorTreeF,  caracter=''):
    ...
    tamanho_Value = max(len(ValorOne), len(ValorTwo), len(ValorTree), len(ValorOneF), len(ValorTwoF), len(ValorTreeF)) # Tamanho máximo entre texto e textoone
...
# Chamando Função para validar sabor. Acertou ela break e vai para o loop 
def validar_sabor():
    while True:
...
# Chamando Função para validar tamanho. Acertou ela break e vai para o loop 
def validar_tamanho():
    while True:
...
#Especificando cada iten da tabela acima
texto = "Tamanho"
...
while True:
    # Chamando a Função para validar sabor/tamanho. Digitado no input desce para passar pelo loop
    sabor = validar_sabor()
    ...
    #No final do loop esta o parametro dele. Acumula de preço em preço.(Preço anterior + preço atual)
    acumulador += preço
